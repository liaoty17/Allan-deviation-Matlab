function [Out_tau, OHDEV_Output, Out_ErrBar, alpha_memory] = Hadamard_OHDEV(LiveData, tau0, Phase_or_Fre, tau, Conf_interval, Noise_type)
% Overlapping Hadamard deviation calculation.
 % 
...
%% Input parameters
    ...
    if strcmp(Phase_or_Fre, "Frequency")
        ...
        for i=1:1:len_tau
            ...
            LiveData_Average_SQ = mean(Delta_yData_bar_Diff2.^2);       
            OHDEV_Output(i,1) = sqrt(LiveData_Average_SQ/6);
            ...
            edf_ohdev = EDF_Numerical_sim(log_slope_alpha_approximate,3,m(i),m(i),m(i),N+1);
            Out_ErrBar(i, 1:2) = OHDEV_Output(i,1).*[sqrt(edf_ohdev/chi2inv(0.5+Conf_interval/2, edf_ohdev)), sqrt(edf_ohdev/chi2inv(0.5-Conf_interval/2, edf_ohdev))];
        end
    ...
    elseif strcmp(Phase_or_Fre, "Phase")
        ...
        for j=1:1:len_tau
            ...
            Delta_yData_bar = (LiveData_yBar_Diff2).^2;
            OHDEV_Output(j,1) = sqrt((mean(Delta_yData_bar)/(Out_tau(j)^2)/6));
            ...
            edf_ohdev = EDF_Numerical_sim(log_slope_alpha_approximate, 3, m(j), m(j), m(j), N);
            Out_ErrBar(j, 1:2) = OHDEV_Output(j,1).*[sqrt(edf_ohdev/chi2inv(0.5+Conf_interval/2, edf_ohdev)), sqrt(edf_ohdev/chi2inv(0.5-Conf_interval/2, edf_ohdev))];
        end
